    ...
    <div class="row">
                ...
                @if($companies->count() > 0)
                    ...
                    @auth
                        <div class="text-right">
                            {{ __('Or create a new Company')}}: <a class="btn btn-success" href="{{route('company_create')}}">{{ __('Create')}}</a>
                        </div>
                    @endauth
                @else
        ...
        <div class="col-md-12">
            ...
            @if($employees->count() > 0)
                ...
                <table class="table table-striped table-bordered table-hover table-condensed">
                    ...
                    @foreach($employees as $employee)
                        ...
                        <tr>
                            ...
                            @auth
                                <td>
                                    <a href="{{route('employee_edit',$employee->id)}}" class="btn btn-primary btn-sm">{{ __('Edit')}}</a>
                                    <a href="{{route('employee_destroy',$employee->id)}}" class="btn btn-danger btn-sm" onclick="return ConfirmDelete('Are you sure you want to delete?');">{{ __('Remove')}}</a>
                                </td>
                            @endauth
                        </tr>
                ...
                @auth
                    <div class="text-right">{{ __('Or create a new Employee')}}: <a class="btn btn-success" href="{{route('employee_create')}}">{{ __('Create')}}</a></div>
                @endauth
